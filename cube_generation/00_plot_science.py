...
points = [{"latitude": float(y[i]), "longitude": float(x[i])} for i in range(len(x))]

# Create a map
fig, ax = plt.subplots(subplot_kw={"projection": ccrs.PlateCarree()})
ax.set_extent([-4, 30, 40, 65], crs=ccrs.PlateCarree())
...
plt.tight_layout()
plt.savefig(os.path.join(_DIR, "plots", "sciencecubes_location"))
plt.close()
